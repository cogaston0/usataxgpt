...
<body data-page="home">
  ...
  <!-- Header and footer injected by template.js -->
  ...
  <!-- Load template (injects header + footer + hamburger) -->
  ...
  <script>
    ...
    function clearQuickReplies() {
      ...
      multiSelections = []; // safe here — Done button snapshots before calling handleAnswer
    }
    function setInputDisabled(disabled) {
      ...
      if (!disabled) {
        ...
        userInput.placeholder = "Please choose an option above…";
      }
    }
      ...
      if (step.type === "quick") {
        ...
        if (step.multi) {
          ...
          doneBtn.addEventListener("click", function(e) {
            e.stopPropagation();
            // Read and snapshot selections DIRECTLY - bypass handleAnswer entirely
            var val = multiSelections.length ? multiSelections.slice().join(", ") : "None selected";
            var savedKey  = currentStep ? currentStep.key  : null;
            var savedNext = currentStep ? currentStep.next : "done";

            if (savedKey) answers[savedKey] = val;
            addMsg(val, "user");
            quickReplies.innerHTML = "";
            multiSelections = [];
            userInput.value = "";
            setInputDisabled(true);
            goToStep(typeof savedNext === "function" ? savedNext(val) : savedNext);
          });
    ...
    function showCompletion() {
      ...
      setTimeout(function() {
        ...
        addMsg("Thank you, <strong>" + escapeHTML(name) + "</strong>! 🎉 Your intake is complete. Here is a summary:", "bot");
